...
public class GroupExitTask extends AsyncTask<Void, Void, Boolean> {
    ...
    public GroupExitTask(Group group) {
        this.group = group;
    ...
    protected Boolean doInBackground(Void... params) {
        ...
        try {
            ...
            httppost = new HttpPost(Util.SERVER_ADDRESS + "group_exit.php");
            ...
            nameValuePairs.add(new BasicNameValuePair("login_id", firstFragment.meInfo.get(0).getId()));
    ...
    protected void onPostExecute(final Boolean success) {
        ...
        if (success) {
            for (int i = 0; i < firstFragment.arrayListGroup.size(); i++) {
                if (firstFragment.arrayListGroup.get(i).getGroups_id().equals(group.getGroups_id())) {
                    firstFragment.arrayListGroup.remove(i);
            ...
            firstFragment.mToast.setText("그룹나가기 완료");
            firstFragment.mToast.show();
