...
public class MarkerInfoRemoveTask extends AsyncTask<Void, Void, Boolean> {

    ShowDetailActivity context;

    // 유저가 입력한 id와 pw를 저장할 변수
    private int index;
    ...
    public MarkerInfoRemoveTask(ShowDetailActivity context, int index) {
        this.index = index;
    ...
    protected Boolean doInBackground(Void... params) {
        ...
        try {
            ...
            httppost = new HttpPost(Util.SERVER_ADDRESS + "info_remove.php");
    ...
    protected void onPostExecute(final Boolean success) {
        ...
        if (success) {
            Log.e("삭제", "성공");
            Model.myMarkerItems.remove(index);
            MapLoadActivity.mapLoadActivity.clickedMarker.remove();


            ShowDetailActivity.mToast.setText("삭제 되었습니다.");
            ShowDetailActivity.mToast.show();

            ShowDetailActivity.showDetailActivity.finish();


        } else {
            Log.e("삭제", "실패");
            ShowDetailActivity.mToast.setText("삭제 할 수 없습니다.");
            ShowDetailActivity.mToast.show();
        }
