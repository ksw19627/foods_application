...
public class GroupNameChangeTask extends AsyncTask<Void, Void, Boolean> {
    ...
    public GroupNameChangeTask(Group group, String changeGroupName, String loginUserId) {
        this.group = group;
    ...
    protected Boolean doInBackground(Void... params) {
        ...
        try {
            ...
            Log.e("group id ", group.getGroups_id());
            Log.e("group name ", changeGroupName);
            Log.e("user id ", loginUserId);
            // http방식을 통한 서버주소 + php파일의 경로를 설정
            // Simulate network access.
            httpclient = new DefaultHttpClient();
            httppost = new HttpPost(Util.SERVER_ADDRESS + "group_name_change.php");
            ...
            nameValuePairs.add(new BasicNameValuePair("changeGroupName", changeGroupName));
            nameValuePairs.add(new BasicNameValuePair("loginUserId", loginUserId));
    ...
    protected void onPostExecute(final Boolean success) {
        ...
        if (success) {
            Log.e("group명 변경", "성공");
            FirstFragment context = FirstFragment.getInstance();
            group.setGroups_name(changeGroupName);
            context.textGroupName.setText(changeGroupName);

            Util.groupArrayListSort(context.arrayListGroup);
            context.groupAdapter.notifyDataSetChanged();

            context.mToast.setText("그룹명 변경 완료!");
            context.mToast.show();
