...
public class MarkerInfoTakeTask extends AsyncTask<Void, Void, Boolean> {
    ...
    public MarkerInfoTakeTask(ShowDetailActivity context, int index) {
        this.index = index;
    ...
    protected Boolean doInBackground(Void... params) {
        ...
        try {
            ...
            httppost = new HttpPost(Util.SERVER_ADDRESS + "info_take.php");

            // 로그인입력값인 id와 pw를 input으로 설정하여 ArrayList에 set한다.
            nameValuePairs = new ArrayList<NameValuePair>(2);

            nameValuePairs.add(new BasicNameValuePair("login_id", MainActivity.context.loginUser.getId()));
    ...
    protected void onPostExecute(final Boolean success) {
        ...
        if (success) {
            Log.e("가져오기", "성공");
            ShowDetailActivity.mToast.setText("내 지도에 복사되었습니다.");
            ShowDetailActivity.mToast.show();

        } else {
            Log.e("가져오기", "실패");
            ShowDetailActivity.mToast.setText("이미 내 지도에 존재합니다.");
            ShowDetailActivity.mToast.show();
        }
